...
describe('installPyPy', () => {
  ...
  it('throw if release is not found', () => {
    ...
    expect(spySymlinkSync).not.toHaveBeenCalled();
    expect(spyExistsSync).not.toHaveBeenCalled();
    expect(spyExec).not.toHaveBeenCalled();
    expect(spyFsWriteFile).not.toHaveBeenCalled();
    expect(spyFsReadDir).not.toHaveBeenCalled();

    if (IS_WINDOWS) {
      expect(spyExtractZip).not.toHaveBeenCalled();
    } else {
      expect(spyExtractTar).not.toHaveBeenCalled();
    }
  });

  it('found and install PyPy', () => {
    expect(() => installer.installPyPy('7.3.x', '3.6.12', 'x64')).toEqual({
    ...
    expect(spyHttpClient).toHaveBeenCalled();
    expect(spySymlinkSync).toHaveBeenCalled();
    expect(spyExistsSync).toHaveBeenCalled();
    expect(spyExec).toHaveBeenCalled();
    expect(spyFsWriteFile).toHaveBeenCalled();
    expect(spyFsReadDir).toHaveBeenCalled();

    if (IS_WINDOWS) {
      expect(spyExtractZip).toHaveBeenCalled();
    } else {
      expect(spyExtractTar).toHaveBeenCalled();
    }
  });
});
